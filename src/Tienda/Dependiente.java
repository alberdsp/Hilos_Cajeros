...
/*
 *  clase que define a las dependientas/es, lo hacemos runeable
 *  e incorporamos hilos para poder tener un hilo por cajera
 */
	...
	public void run() {
		...
		for (int i = 0; i < cliente.getCompra().length; i++) {
			...
			try {
				...
				System.out.println("PRODUCTO " + (i + 1) + " " + " pasa por caja " + caja + " en "
						+ (System.currentTimeMillis() - tiempo) + "ms");
		...
		System.out.println("------------------------------------------------------------------");
		System.out.println("COMPRA DEL CLIENTE: " + cliente.getNombre() + " PROCESADA EN "
				+ (System.currentTimeMillis() - tiempo) + "ms");
		System.out.println("------------------------------------------------------------------");
